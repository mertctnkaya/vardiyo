...
Phase 1: Security Hardening & Backend Defense[ ] RLS Policy Validation: Run SQL policy audit across all 5 public tables to verify that both qual and with_check clauses enforce auth.uid() = user_id.[ ]
Service Key Scrubbing: Verify .env, .env.example, and Git commit history contain zero instances of SUPABASE_SERVICE_ROLE_KEY.
[ ] CORS Hardening: Restrict Supabase API allowed origins strictly to production domain (https://vardiyo.vercel.app), local development (http://localhost:5173), and native capacitor scheme (capacitor://localhost).
[ ] Edge Function Rate Limiting: Add token/IP throttling to admin-broadcast and send-push to prevent denial-of-wallet / quota exhaustion.
[ ] Server-Side Validation: Re-validate form constraints (dates, wage numbers, hours) inside Supabase table CHECK constraints or database triggers.Phase 2: Technical SEO & Browser Identity
[ ] Dynamic Meta Titles & Descriptions: Implement dynamic page titles in React Router (document.title = ...) for every sub-route (/worktime, /calculations, /faq, /settings, /contact).
[ ] Robots & Sitemap: Create public/robots.txt and public/sitemap.xml mapping all public pages.
[ ] Image Accessibility: Audit all <img> elements across views to ensure meaningful Turkish alt text.
[ ] Open Graph Absolute URL: Update index.html og:image to point to full production URL (https://.../vardiyo.png).Phase 3: UX/UI Polish & Resilience[ ] Custom 404 Page: Create a branded NotFound.tsx route matching #16191d aesthetics with a "Return Home" CTA.
[ ] Loading Skeletons: Implement loading skeletons for ShiftDisplayCard, CalendarGrid, and PayrollSection during data fetch states. [ ] Form Error States: Ensure input elements display standard red borders (border-red-500/50) and inline validation helper texts upon invalid input.
[ ] Sticky Mobile CTAs: Ensure primary actions (e.g., "Ayarları Kaydet", "Hesapla") adhere to viewport bottoms on mobile screens.
[ ] Thank-You States: Provide explicit modal / alert confirmations following successful contact message submissions.Phase 4: Legal, Compliance & Store Readiness
[ ] Resignation / Severance Rights Matrix (Turkish Labor Law):Integrate the Termination Codes table into src/constants/faqData.ts and display in src/components/faq/RightsTab.tsx:Kod 3 (İstifa - Kendi İsteğiyle): Kıdem: ALAMAZ, İşsizlik: ALAMAZ, İhbar: ALAMAZKod 3 (Şahsi / Ailevi Nedenler, Tayin): Kıdem: ALAMAZ, İşsizlik: ALAMAZ, İhbar: ALAMAZKod 4 (Haksız İşten Çıkarılma): Kıdem: ALIR, İşsizlik: ALIR, İhbar: ALIRKod 25 (Görülen Lüzum Üzerine): Kıdem: ALIR, İşsizlik: ALIR, İhbar: ALAMAZKod 25 (Haklı Fesih / Mobbing / Sağlık): Kıdem: ALIR, İşsizlik: ALIR, İhbar: ALAMAZKod 12 (Askerlik Nedeniyle Erkekler): Kıdem: ALIR, İşsizlik: ALIR, İhbar: ALAMAZKod 13 (Evlilik Nedeniyle Kadınlar - 1 Yıl İçinde): Kıdem: ALIR, İşsizlik: ALAMAZ, İhbar: ALAMAZKod 14 (08.09.1999 Öncesi - 15 Yıl / 3600 Gün): Kıdem: ALIR, İşsizlik: ALAMAZ, İhbar: ALAMAZKod 14 (09.09.1999 Sonrası - 25 Yıl / 4500-7000 Gün): Kıdem: ALIR, İşsizlik: ALAMAZ, İhbar: ALAMAZKod 14 (01.05.2008 Sonrası - 4600-5400 Gün): Kıdem: ALIR, İşsizlik: ALAMAZ, İhbar: ALAMAZKod 16 (Başka İşyerine Nakil): Kıdem: ALAMAZ, İşsizlik: ALAMAZ, İhbar: ALAMAZKod 17 (İşyeri Kapandığı İçin): Kıdem: ALIR, İşsizlik: ALIR, İhbar: ALIRKod 18 (Sağlık Nedeninden Fesih): Kıdem: ALIR, İşsizlik: ALIR, İhbar: ALAMAZ[ ] Privacy Policy & Terms of Service: Add legal pages required by Google Play Store & KVKK regulations.[ ] Cookie Consent Banner: Add non-intrusive cookie / local storage disclosure banner.[ ] Analytics Setup: Integrate privacy-friendly analytics (PostHog or Google Analytics).
