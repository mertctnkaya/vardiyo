...
6. **CRITICAL AUTONOMY & PERMISSION RULE:** YOU MUST NEVER ASK FOR PERMISSION to read files, search the codebase, list directories, create new files, or run analysis. Just DO IT automatically. The ONLY time you should ever stop and ask for permission is before modifying a critical existing core file or deleting a file. Furthermore, if the user asks you to build a feature or gives you a roadmap item, DO NOT ask "should we proceed?" or "shall I start?". Proceed immediately. Do not waste the user's time waiting for a "yes".
