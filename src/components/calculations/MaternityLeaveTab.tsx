...
export default function MaternityLeaveTab() {
  ...
  const calculateMaternity = () => {
    if (!grossSalary || isNaN(Number(grossSalary)) || !birthDate) return null;
    ...
    // Analık ödeneği brütün 2/3'ü üzerinden hesaplanır
    const totalAllowance = (dailyGross * (2 / 3)) * totalDays;

    // Beklenen tarihten 8 hafta önce ayrılır, 8 hafta sonra biter
    ...
    // Süt izni bitişi (Doğumdan 1 yıl sonra)
  ...
  const result = calculateMaternity();
