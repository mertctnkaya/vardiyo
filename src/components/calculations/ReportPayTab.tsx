...
export default function ReportPayTab() {
  ...
  const calculateReportPay = () => {
    if (!grossSalary || isNaN(Number(grossSalary)) || !reportDays || isNaN(Number(reportDays))) return null;

    const gross = Number(grossSalary);
    const days = Math.floor(Number(reportDays));

    if (days < 3) {
      return { error: 'SGK, 3 günden az (1 veya 2 günlük) raporlar için işgöremezlik ödeneği yatırmaz. Bu günlerin ücreti işveren insiyatifindedir.' };
    }

    const dailyGross = gross / 30;
    const paidDays = days - 2; // İlk 2 gün SGK tarafından ödenmez
  ...
  const result = calculateReportPay();
      ...
      <div className="bg-base-200 p-4 sm:p-6 rounded-2xl border border-base-300 flex flex-col justify-center min-h-[300px]">
        {result ? (
          ...
          <div className="text-center opacity-50 flex flex-col items-center">
            ...
            <p className="font-medium text-sm sm:text-base">Hesaplama için maaş ve rapor günü girin.</p>
          </div>
        )}
      </div>
    </div>
