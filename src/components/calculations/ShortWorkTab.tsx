...
export default function ShortWorkTab() {
  ...
  const calculateWorkAllowance = () => {
    if (!grossSalary || isNaN(Number(grossSalary))) return null;

    const gross = Number(grossSalary);

    if (workType === 'kisa') {
      let allowance = gross * 0.60;
      const maxLimit = CURRENT_GROSS_MIN_WAGE * 1.50; // Asgari ücretin %150'sini geçemez
      ...
      // Yarım çalışma (Örn: Doğum sonrası) - Maaşın yarısını işveren, kalanını asgari ücret üzerinden İŞKUR öder.
      const employerHalfGross = gross / 2;
      // İŞKUR kısmı asgari ücretin yarısıdır (Damga vergisi kesilir)
  ...
  const result = calculateWorkAllowance();
      ...
      <div className="bg-base-200 p-4 sm:p-6 rounded-2xl border border-base-300 flex flex-col justify-center min-h-[300px]">
        {result ? (
          ...
          <div className="text-center opacity-50 flex flex-col items-center">
            ...
            <p className="font-medium text-sm sm:text-base">Maaş bilginizi girerek ödenek desteğini görüntüleyin.</p>
          </div>
        )}
      </div>
    </div>
